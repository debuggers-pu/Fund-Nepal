    ...
    <div className="flex justify-between items-center bg-green-500 text-white py-2 px-10 ">
      ...
      {isAuthenticated === true ? (
        <div className="flex items-center space-x-2">
         <NavLink to="/">
          <button className="px-4 py-2 bg-white text-primaryColor hover:opacity-75 rounded-sm border-none" onClick={onClick}>Logout</button>
          </NavLink>
          <img src={`http://localhost:5000/${image.split("\\")[1]}`} className="h-8 w-8 rounded-full"/>
        </div>
      ) : (
